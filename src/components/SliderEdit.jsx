...
export default function Home() {
    ...
    const { user, introVideo, userDB, option,  setUserSuccess, success, setUserData, postsIMG, setUserPostsIMG, item, cliente, setCliente, cart, setCart, modal, setModal } = useUser()
    ...
    useEffect(() => {
        if (Object.keys(data3).length === 0 && cliente && cliente[option] && cliente[option] && cliente[option]) {
            console.log('getData3')
    ...
    return (
        <form className="relative  pt-10" onSubmit={saveFrontPage2} >
            <Link href={`/Admin/Edit/AddTarjetasSlider?item=${query}`} className='fixed bottom-[100px] right-[100px]  rounded-full z-50 block font-medium '>
            ...
            {data3 && data3 !== undefined && Object.entries(data3).map((i) => {
                return <div className="">
                    ...
                    {itemEdit === i[0] && <div className='space-y-5'>
                        ...
                        < InputFlotante type="text" name={`companyURL`} id={`floating_6`} onChange={(e) => onChangeHandler3(e, i[0])} value={data3[i[0]] && data3[i[0]]['companyURL'] ? data3[i[0]]['companyURL'] : i[1][`companyURL`]} required label={'URL de empresa'} shadow='shadow-white' />
                        <br />
                        <div className='flex justify-center'>
                            <Button type="button" theme="Danger" click={(e) => deleteHandler(e, `Cliente/${option}/${i[0]}`, i[0], setData3)}>Eliminar</Button>
