...
export default function Section({ subtitle, subtitleEN, description, descriptionEN, video, gradiente, id, children, tarjetas, miniTarjetas, especial }) {
  ...
  return <section className='relative w-full    bg-gradient-to-tr from-[#3e3f52dc] via-[#06102bdc] to-[#323147dc] overflow-x-hidden overflow-hidden' id={id}
    // style={{
  ...
  >
    ...
    <div className='relative px-5 py-12 w-full lg:px-[100px]  z-30    from-[#d1d1d1dc] via-[#00195cb6] to-[#00195cdc] '>
      <div>
        ...
        >
          {descriptionEN
            ...
            >
              <Translator from='es' to={languaje.slice(0, 2).toLowerCase()}>
                {parse(description.replaceAll("<s>", "<span style='text-transform: uppercase; font-weight: 500'>").replaceAll("</s>", "</span>"))}
                </Translator>
      ...
      {/* {!especial && <div className='flex w-full justify-start '>
        <button type="button" className="w-full border-[2px] md:max-w-[300px] text-gray-900 bg-[#ffd900] hover:bg-[#ffd900]/90 focus:ring-4 focus:outline-none focus:ring-[#ffd900]/50 font-medium rounded-lg text-[12px] px-5 py-2.5 text-center inline-flex items-center " onClick={() => redirectHandlerWindow(`https://api.whatsapp.com/send?phone=${cliente.contactos.celular.replaceAll(' ', '')}&text=hola%20Logistics%20Gear,%20quiero%20ordenar%20un%20servicio%20${subtitle}%20`)}>
    ...
    <div className='relative min-h-screen  w-full flex flex-col justify-top lg:flex-wrap  lg:flex-row lg:justify-center lg:items-center  z-20  '>
      ...
      <div className='absolute top-0 w-full min-h-[100vh] h-full object-cover z-20  bg-gradient-to-tr from-[#3e3f52dc] via-[#06102bdc] to-[#323147dc] '></div>
